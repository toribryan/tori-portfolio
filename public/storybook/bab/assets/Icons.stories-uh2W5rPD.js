...
}`,...H.parameters?.docs?.source},description:{story:`One library, one weight. Every glyph is Lucide, drawn at one stroke that
scales with the icon, so a 12px check and a 40px robot keep the same
stroke-to-size ratio and read as the same family.`,...H.parameters?.docs?.description}}},U.parameters={...U.parameters,docs:{...U.parameters?.docs,source:{originalSource:`{
...
}`,...U.parameters?.docs?.source},description:{story:`Selection changes colour, not the glyph. Unselected destinations sit in the
muted foreground; the selected one steps to the full foreground on its chip.
Weight and fill never change, so every icon reads the same.`,...U.parameters?.docs?.description}}},W=[`Weight`,`SelectedState`]})))()}G();export{U as SelectedState,H as Weight,W as __namedExportsOrder,V as default};