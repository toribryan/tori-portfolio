---
title: Token Flow
description: A colour token walking from raw value to primitive to semantic utility, with a pulse along each wire. Theme aware.
...
- One row per colour, three chips wired left to right: the raw value, the primitive that names it, and the semantic utility that uses it.
...
Each row's `base` also colours the swatch on its three chips, so a row that swaps in the dark theme changes colour along with its labels.
...
| ----------- | ------------------- | ------- | --------------------------------------------------------------- |
| `rows`      | `TokenRow[]`        |         | One entry per colour.                                           |
